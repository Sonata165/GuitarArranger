...
def generate_all_possible_positions():
    ...
    for string in range(1, 7):
        ...
        config = infer_info(tab, finger_used, 0)
    ...
    chord_and_fingers = [
        ...
        (['x','x',0,2,3,2], [0,0,0,2,3,1], 1),      # D
        ([0,2,2,1,0,0], [0,2,3,1,0,0], 1),          # E
        ([1,3,3,2,1,1], [1,3,4,2,1,1], 1),          # F
        ([3,2,0,0,0,3], [3,2,0,0,0,4], 1),          # G
        (['x',0,2,2,2,0], [0,0,1,2,3,0], 1),        # A
        (['x','x',4,4,4,2], [0,0,2,3,4,1], 1),      # B
        (['x','x',5,5,4,3], [0,0,3,4,2,1], 3),      # Cm
        (['x','x',0,2,3,1], [0,0,0,2,3,1], 1),      # Dm
        ([0,2,2,0,0,0], [0,2,3,0,0,0], 1),          # Em
        ([1,3,3,1,1,1], [1,3,4,1,1,1], 1),          # Fm
        (['x','x',5,3,3,3], [0,0,3,1,1,1], 3),      # Gm
        (['x',0,2,2,1,0], [0,0,2,3,1,0], 1),        # Am
        (['x',2,4,4,3,2], [0,1,3,4,2,1], 2)         # Bm
    ]
    for chord, fingers, position in chord_and_fingers:
        ...
        for k in range(1, 6):
            for subset in itertools.combinations(string_indices, k):
                ...
                config = infer_info(tab_sub, fingering_sub, position)
                positions.append(config)
        # Add the full chord itself
        config = infer_info(chord, fingers, position)
        positions.append(config)

    ''' CAGED chord positions '''
    chord_and_fingers = [
        (['x',4,3,1,2,1], [0,4,3,1,2,1], 1),        # C shape
        (['x',1,3,3,3,'x'], [0,1,2,3,4,0], 1),      # A shape
        ([4,3,1,1,1,4], [3,2,1,1,1,4], 1),          # G shape
        ([1,3,3,2,1,1], [1,3,4,2,1,1], 1),          # E shape
        (['x','x',1,3,4,3], [0,0,1,2,4,3], 1),      # D shape
        (['x',4,2,1,2,'x'], [0,4,2,1,3,0], 1),      # C shape (minor)
        (['x',1,3,3,2,1], [0,1,3,4,2,1], 1),        # A shape (minor)
        (['x','x',1,4,4,4], [0,0,1,3,3,3], 1),      # G shape (minor)
        ([1,3,3,1,1,1], [1,3,4,1,1,1], 1),          # E shape (minor)
        (['x','x',1,3,4,2], [0,0,1,3,4,2], 1),      # D shape (minor)
    ]
    for chord, fingers, _ in chord_and_fingers:
        for position in range(1, HIGHEST_POSITION + 1):
            ...
            for k in range(1, 6):
                for subset in itertools.combinations(string_indices, k):
                    ...
                    positions.append(config)
            # Add the full chord itself
            config = infer_info(tab, fingers, position)
            positions.append(config)
...
fretboard = Fretboard()
def infer_info(tab, finger_used, position):
    '''
